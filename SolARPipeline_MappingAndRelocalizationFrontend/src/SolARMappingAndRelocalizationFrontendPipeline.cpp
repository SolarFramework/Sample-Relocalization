...
void SolARMappingAndRelocalizationFrontendPipeline::processRelocalization()
{
    ...
    try {
        ...
        auto curTSolAR = get3DTransformSolAR(clientContext);
        if (!curTSolAR.isApprox(Transform3Df::Identity()))  // if T defined compute coarse pose in SolAR
            poseCoarse = curTSolAR*pose;
        if (clientContext->m_relocalizationService->relocalizeProcessRequest(image, new_pose, confidence, poseCoarse) == SolAR::FrameworkReturnCode::_SUCCESS) {
            ...
            if (clientContext->m_mappingStatus != BOOTSTRAP) {
                ...
                if ((clientContext->m_cumulativeDistance > m_minCumulativeDistance) && (dist.norm() > clientContext->m_cumulativeDistance*m_thresTranslationRatio)) {
                    ...
                    clientContext->m_cumulativeDistance = 0.f; // reset cumulative distance
                    return;
                }
            }
            else {
