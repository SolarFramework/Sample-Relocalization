    ...
    public api::pipeline::IAsyncRelocalizationPipeline
{
  ...
  private:
    ...
    SolAR::datastructure::Transform3Df  m_T_M_SolAR, m_T_M_SolARInit;  // transform from Device (AR runtime) to SolAR
    ...
    float m_poseDisparityTolerance = 0.15f;   // when initializing the first transform device to solar, the list of reloc transforms must be close to each other in translation
